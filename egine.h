...
void app_phys(std::vector<Ball> *balls, float GG, float G, float time) {
    float s1, s2;
    for (int k = 0; k < 3; k++) {
...
void analise_map(std::vector<Ball> *balls) {
    float s1, s2;
...
void smart_push_ball(std::vector<Ball> *balls) {
    int ran;
    int n1, d1, n2, d2;
    n1 = 73;
    d1 = 10;
...
void create_first_balls(std::vector<Ball> *balls) {
    balls->clear();
    for (int i = 1; i < 6; i++) {
...
int score(std::vector<Ball> *balls) {
    int ret = 0;
    for (int i = 0; i < (*balls).size(); i++) {
        ret += (*balls)[i].st;
    }
